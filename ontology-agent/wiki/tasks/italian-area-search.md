---
id: entity_7e911538fc1a0eb2
type: Task
project: ontology-agent
graph_node_id: entity_7e911538fc1a0eb2
sources:
- span_63ffe0bbb61cea27
---

# Task: Italian area search

## Summary

Italian area search is tracked as a Task in the project graph. It has 2 outgoing and 1 incoming validated relationships.

## Aliases

- None

## Outgoing Relationships

- `uses` -> [[include-filters|Include filters]] (confidence 0.90, validated)
- `uses` -> [[negative-filters|Negative filters]] (confidence 0.95, validated)

## Incoming Relationships

- [[daily-flow|Daily Flow]] -> `contains` (confidence 0.98, validated)

## Evidence

- `span_63ffe0bbb61cea27`: # WebSmith Operating Playbook  This is the daily workflow reference. Keep it short, operational, and source-backed.  ## Daily Flow  1. Search an Italian area by location, radius, optional business name, and optional activity types. 2. Us...
